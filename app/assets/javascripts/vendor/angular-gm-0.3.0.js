...
(function() {
  ...
  value('angulargmDefaults', {
    ...
    'mapOptions': {
      zoom : 8,
      center : new google.maps.LatLng(31.570686, 34.607732),
...
(function () {
  ...
  directive('gmInfoWindow', 
    ...
    function link(scope, element, attrs, controller) {
      ...
      if (!infoWindow) {
        infoWindow = new InfoBubble(opts);
        infoWindow.setMinHeight(180);
        infoWindow.setMinWidth(530);
...
;
      ...
      infoWindow.open = function open(map, anchor,id) {
        $compile(element.contents())(scope);
          //console.log('window opened',scope,id);
          //var htm = angular.element('.kpis[data-pid='+id+']').html();

              //console.log('window');
              _open.call(infoWindow, map, anchor);
          // $timeout(function(){
          //     var innerPlots =  document.getElementById("values");
          //     innerPlots.style.display="none";
          //     innerPlots.innerHTML = htm;
          //     $('#values').fadeIn("slow");
          // },200);
        ...
        $timeout(function () {
          if (hasCenter || hasZoom || hasBounds || hasMapTypeId) {
            scope.$apply(function (s) {
              ...
              if(!scope.$root.firstRender){
//                  debugger;
                  scope.$broadcast('gmMapResize','myMap');
                  scope.$root.firstRender=true;
              }
            });
          }
        });
      ...
      scope.$on('gmRender',function(event,gmMapId){
