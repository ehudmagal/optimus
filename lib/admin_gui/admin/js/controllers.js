    ...
    $http.get( $scope.orders_url)
        .success(function (data, status, headers, config) {
            ...
            for (var i = 0; i < $rootScope.orders.length; i++) {
                ...
                var content_string = 'user: ' + transit.user_email + '<br>';
                content_string += 'source address: ' + transit.source.address + '<br>';
                content_string += 'destination address: ' + transit.destination.address + '<br>';
    ...
    $scope.marker_cliked_event = function () {
        ...
        if (!this.dest_marker) {
            ...
            this.dest_marker = new google.maps.Marker({
                position: {lat: lat, lng: lng},
                map: $scope.map,
                title: transit.destination.address,
