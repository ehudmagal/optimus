...
app.controller('OrdersCtrl', function ($scope, $rootScope, $filter, $http, ngTableParams, breadcrumbs, $location,
                                       $cookies,$routeParams ,GetObjectByIdFactory, GetObjectByUrlPaginationFactory,
                                       GetManyToOneArrayFactory, GetObjectFromCacheFactory,GetObjectByAttributeFactory,
                                       CenterOfCoordsService) {
    $scope.breadcrumbs = breadcrumbs;
    $scope.location = $location;
    $scope.personal_orders = $routeParams.personal_orders;
    $scope.isViewLoading = false;
        ...
        $scope.setSourceDest = function () {
            if ($scope.markers.length < 2) {
                ...
                if (!$scope.order.source) {
                    ...
                    $.blockUI({
                        message: $('#project_form'), css: {
                            top: '10px',
                            'min-width': '700px'
                        },
                        onOverlayClick: $scope.end_edit_order
                    });
                }
                $("#address").val('');
                $scope.markers.push(marker);
