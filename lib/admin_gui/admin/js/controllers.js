...
app.controller('orderConfigurationCtrl', ['$scope', '$routeParams', '$http', '$cookies', 'loggerRepository', 'UserLoggersRepository',
    ...
    function ($scope, $routeParams, $http, $cookies, loggerRepository, UserLoggersRepository, ChangeLoggerOwnerFactory,
        ...
        $scope.goto_batch = function(batch){
            var url = '#/orders/'+$scope.orderId+'/batches/'+batch.id;
            window.location.assign(url);
        }

        $scope.load_data = function(){
            if($scope.orderId){
                $http.get(server_string + '/admin/orders/' + $scope.orderId + '.json', {
                    timeout: 30000,
                    cache: false
                }).success(function (data) {
                    $scope.order = data;
                ...
                $scope.order = {};
                if($scope.companyId){
                    $scope.order.company_id = parseInt($scope.companyId);
                }
            }
            $http.get(server_string + '/admin/companies.json', {
                timeout: 30000,
        ...
        $scope.edit_order = function () {
            var url = server_string + '/admin/orders.json?';
            var post_data = {};
            var csrf_token = $cookies['CSRF-TOKEN'];
            post_data['authenticity_token'] = csrf_token;
            post_data['utf8'] = '%E2%9C%93';
            ...
            var method = 'patch';
            if(!$scope.orderId){
                post_data['commit'] = 'Create Order';
                method = 'post';
            }else{
                post_data['_method'] = 'patch';
                url = server_string + '/admin/orders/'+$scope.orderId+'.json?';
            }
            PatchObjectFactory.patchObject(url,post_data,method, $scope.goto_order_page);
        ...
        $scope.goto_company = function(){
            if($scope.order.company_id){
                window.location.assign("#/companies/" +$scope.order.company_id);
            }
        }
