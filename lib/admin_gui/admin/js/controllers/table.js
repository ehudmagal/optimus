materialAdmin
    .controller('tableCtrl', function($filter, $sce, ngTableParams, tableService,$http,$scope,$rootScope,$compile,
        ...
        $scope.show_bids = function (order) {
            var res = false;
            if(order.bids.length &&  $rootScope.private_zone  ){
                res = true;
            }
            return res;
        }
        ...
        $scope.place_bid = function(){
            var order =  $scope.current_marker.order;
            var price = parseFloat($scope.bid_price);
            var post_data = {
                price: price,
                order_id: order.id
            }
            var url = "/bids.json";
            PatchObjectFactory.patchObject(url, post_data, "post", function () {alert("Bid placed successfully");});
        }
