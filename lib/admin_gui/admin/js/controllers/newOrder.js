...
.controller('newOrderCtrl', function($filter, $sce, ngTableParams, tableService,$http,$scope,$rootScope,
    ...
    $scope.submit_order = function () {
        var url =   '/orders.json';
        var post_data = {};
        if(!IsEmptyObjService.isEmpty($scope.order.work_type.pallet)){
            $scope.order.weight = $scope.order.work_type.pallet.amount * $scope.order.work_type.pallet.weight;
        }
        if(!IsEmptyObjService.isEmpty($scope.order.work_type.bulk)){
            $scope.order.weight =  $scope.order.work_type.bulk.weight;
        }
        ...
        if($scope.markers[0] && $scope.markers[1]) {
            $scope.directionsService.route({
                ...
                if (status === google.maps.DirectionsStatus.OK) {
                    ...
                    $scope.order.fixed_price =   FixedPriceService.fixedPrice(response.routes[0],$scope.order,
                        DistanceService);
                } else {
