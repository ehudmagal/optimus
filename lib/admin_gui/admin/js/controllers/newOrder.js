materialAdmin
    .controller('newOrderCtrl', function ($filter, $sce, ngTableParams, tableService, $http, $scope, $rootScope,
        ...
        $scope.addBoxType = function (order) {
            if (order.box_types.length < 3) {
                var new_box = {"boxes_count": 0, "boxes_length": 0, "boxes_width": 0, "boxes_height": 0};
                order.box_types.push(new_box);
            }
        }

        $scope.remove_box_type = function (box_type) {
            var index = $scope.order.box_types.indexOf(box_type);
            if (index != -1) {
                $scope.order.box_types.splice(index, 1);
            }
        }
