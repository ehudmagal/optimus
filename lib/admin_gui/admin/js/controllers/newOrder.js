...
.controller('newOrderCtrl', function($filter, $sce, ngTableParams, tableService,$http,$scope,$rootScope,
    ...
    $scope.init_combojs_components = function () {
        ...
        $('#deliver_after_time').combodate({
            firstItem: 'name', //show 'hour' and 'minute' string at first item of dropdown
            minuteStep: 30
        });
        $('#deliver_cutoff_time').combodate({
            firstItem: 'name', //show 'hour' and 'minute' string at first item of dropdown
            minuteStep: 30
        });
    }
    ...
    $scope.submit_order = function () {
        ...
        $scope.order.pickup_cutoff_time = new Date($scope.order.pickup_time.getMilliseconds() + pickup_cutoff_hour);
        var pickup_hour = $scope.get_milliseconds_by_time($scope.order.pickup_hour);
        $scope.order.pickup_time = new Date($scope.order.pickup_time.getMilliseconds() + pickup_hour);
        var delivery_cutoff_hour =  $scope.get_milliseconds_by_time($scope.order.delivery_cutoff_hour);
        $scope.order.delivery_cutoff_time = new Date($scope.order.delivery_time.getMilliseconds() + delivery_cutoff_hour);
        var delivery_hour = $scope.get_milliseconds_by_time($scope.order.delivery_hour);
        $scope.order.delivery_time = new Date($scope.order.delivery_time.getMilliseconds() + delivery_hour);
